...
public class QueryWord extends QueryInterface {
	...
	public Query generateQuery() {
		return new WildcardQuery(new Term("content", (prefix > 0 ? "*" : "")
				+ term + (suffix > 0 ? "*" : "")));
	}
	...
	public String getFullPatternString(String word_matcher) {
		String pattern_string = "[^" + word_matcher + "]+("
				+ getPatternString(word_matcher, prefix) + term
				+ getPatternString(word_matcher, suffix) + ")[^" + word_matcher
				+ "]+";
	...
	public String getWordPatternString(String word_matcher) {
		return getPatternString(word_matcher, prefix) + term
				+ getPatternString(word_matcher, suffix);
	}
