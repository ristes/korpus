...
public class DetailedResults {
	...
	public QueryInterface getQueryInterface() {
		if (words.size() == 1) {
			return new QueryWord(0, 0, words.get(0));
		} else {
			return new MultipleWordsQuery(getQueryWords(words),
					new ArrayList<Integer>(Collections.nCopies(
							words.size() - 1, 0)), new ArrayList<Integer>(
							Collections.nCopies(words.size() - 1, 1)));
		}
	}
	...
	public static String getHighlited(String content, Matcher phrase_matcher,
			int k) {
		System.out.println(content);
		int start = phrase_matcher.start();
		int end = phrase_matcher.end();
		return content.substring(kWordsBefore(content, k, start), start)
				+ "<b>" + content.substring(start, end) + "</b>"
				+ content.substring(end, kWordsAfter(content, k, end));
	}

	public static int kWordsAfter(String content, int k, int end) {
		int count = 0;
		int i = end;
		int last = i - 1;
	...
	public static int kWordsBefore(String content, int k, int start) {
		int count = 0;
		int i = start;
		int last = i + 1;
		while (count < k && i > 0) {
	...
	public static ArrayList<String> getResults(IndexSearcher searcher,
			String searched, int max) throws IOException {
